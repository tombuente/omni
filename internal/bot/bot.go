...
func (b Bot) voiceStateUpdateCreatorChannel(s *dgo.Session, event *dgo.VoiceStateUpdate) {
	...
	temporaryVoiceChannel, err := s.GuildChannelCreateComplex(
	...
	if _, err := b.db.createTemporaryVoiceChannel(context.Background(), TemporaryVoiceChannel{ID: temporaryVoiceChannel.ID}); err != nil {
		...
		if _, err := s.ChannelDelete(temporaryVoiceChannel.ID); err != nil {
	...
	if err := s.GuildMemberMove(event.GuildID, event.UserID, &temporaryVoiceChannel.ID); err != nil {
...
func (b Bot) voiceStateUpdateTemporaryVoiceChannel(s *dgo.Session, event *dgo.VoiceStateUpdate) {
	...
	beforeUpdateChannelHasUsers := voiceChannelHasUsers(guild, channelID)
	if beforeUpdateChannelHasUsers {
...
func voiceChannelHasUsers(guild *dgo.Guild, channelID string) bool {
	channelHasUsers := false
	for _, voiceState := range guild.VoiceStates {
		if voiceState.ChannelID == channelID {
			channelHasUsers = true
			break
		}
	}

	return channelHasUsers
}
